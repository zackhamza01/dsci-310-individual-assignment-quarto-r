...
We can see @fig-horse-pops that Ontario, Saskatchewan and Alberta 
...
We present the standard deviations in @tbl-horses-pop-sd.


```{r}
#| label: tbl-horses-pop-sd
...
Additionally, note that in @tbl-horses-pop-sd we consider the sample standard deviation of the number of horses during the same time span as @fig-horse-pops.
...
In @fig-largest-sd we zoom in and look at the province of `{r} horses_sd_table$Province[1]`, which had the largest spread of values in terms of standard deviation.
